...
{
  ros::init(argc, argv, "compute_mag_bias");
  ros::NodeHandle nh;

  int n = 0;
  string topic_sensor;// topic_mocap, topic_force;
  ros::param::get("~n", n);
  ros::param::get("~topic_sensor", topic_sensor);

  ros::Subscriber imu_sub = nh.subscribe<sensor_msgs::Imu>(topic_sensor, 2, imu_cb);
